 ...
 * @return bool
 */
function zong_log($msg, $type = 'log') {
    return \zongphp\App::log($msg, $type);
}
...
function browser_log($msg) {
    return \zongphp\App::browserLog($msg);
}
...
function html_in($html) {
    return \zongphp\library\Str::htmlIn($html);
...
function html_out($str) {
    return \zongphp\library\Str::htmlOut($str);
}
...
function html_clear($str) {
    return strip_tags(\zongphp\library\Str::htmlClear($str));
}
...
function str_len($str, $len = 20, $suffix = true) {
    return \zongphp\library\Str::strLen($str, $len, $suffix);
}
...
function int_format($str) {
    return \zongphp\library\Str::intFormat($str);
}
...
function price_format($money) {
    return \zongphp\library\Str::priceFormat($money);
}
...
function price_calculate($n1, $symbol, $n2, $scale = '2') {
    return \zongphp\library\Str::priceCalculate($n1, $symbol, $n2, $scale);
}
...
function load_ui($path = '') {
    $config = \zongphp\Config::get('dux.use');
    if(!$config['ui_local']) {
