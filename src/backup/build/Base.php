...
class Base
{
    ...
    {
        //备份文件夹
        $dir           = Cookie::get('zongphp_backup_dir') ?: $config['dir'];
        $config['dir'] = $dir;
        ...
        if (is_file($dir.'/_config.php')) {
            ...
            Cookie::set('zongphp_backup_dir', $dir);
        ...
        foreach ($cache['table'] as $table => $config) {
            ...
            do {
                ...
                if (empty($data)) {
                    ...
                    foreach ($data as $d) {
                        $sql .= "\zongphp\db\Db::table('{$table}',true)->replace("
                                .var_export($d, true).");\n";
                    }
                }
                //检测本次备份是否超出分卷大小
