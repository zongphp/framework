<?php
namespace zongphp\backup\build;
...
class Base {
	...
	public function backup( array $config, \Closure $callback = null ) {
		//备份文件夹
		$dir           = isset( $_COOKIE['zongphp_backup_dir'] ) ? urldecode( $_COOKIE['zongphp_backup_dir'] ) : $config['dir'];
		$config['dir'] = $dir;
		...
		if ( is_file( $dir . '/_config.php' ) ) {
			...
			setcookie( 'zongphp_backup_dir', urlencode( $dir ) );
			...
			foreach ( $tables['table'] as $table => $data ) {
				...
				$sql .= "\zongphp\db\Db::execute(\"DROP TABLE IF EXISTS {$table}\");\n";
				$sql .= "\zongphp\db\Db::execute(\"{$createSql[0]['Create Table']}\");\n";
			}
			//表结构修改语句
	...
	protected function backupRun( \Closure $callback ) {
		...
		foreach ( $cache['table'] as $table => $config ) {
			...
			do {
				...
				if ( empty( $data ) ) {
					...
					foreach ( $data as $d ) {
						$sql .= "\zongphp\db\Db::table('{$table}',true)->replace(" . var_export( $d, true ) . ");\n";
					}
				}
				//检测本次备份是否超出分卷大小
		...
		setcookie( 'zongphp_backup_dir', 0, 1 );
